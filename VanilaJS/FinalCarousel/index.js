...
const selectedlDot = document.querySelectorAll(".dot");
...
selectedlDot[0].classList.add("selected");
...
const shiftSlide = (dir, action) => {
  ...
  if (allowShift) {
    ...
    selectedlDot[index].classList.remove("selected");
    // next
    if (dir == 1) {
      carouselSlide.style.transform =
        "translateX(" + (posInitial - slideWidth) + "px)";
      index++;
      // prev
    } else if (dir == -1) {
      carouselSlide.style.transform =
...
const checkIndex = () => {
  ...
  if (index == -1) {
    carouselSlide.style.transform =
      "translateX(" + -(slidesLength * slideWidth) + "px)";
  ...
  selectedlDot[index].classList.add("selected");
